...
export const planReviewNotifications = (input: {
  ...
  for (const cycle of input.cycles) {
    ...
    for (const reviewer of voterReviewers(cycle.reviewers)) {
      emails.push({
        html: `<p>Review is open for ${cycle.name}. Please finish your votes before the deadline.</p><p><a href="${origin}/review">Open the review queue</a></p>`,
  ...
  for (const cycle of input.cycles) {
    ...
    for (const reviewer of voterReviewers(cycle.reviewers)) {
      ...
      emails.push({
        html: `<p>Review for ${cycle.name} closes ${when}. You still need to vote on:</p><ul>${titles.map((title) => `<li>${title}</li>`).join('')}</ul><p><a href="${origin}/review">Open the review queue</a></p>`,
  ...
  for (const grant of input.grants) {
    ...
    emails.push({
      html: `<p>All required reviews are in for “${grant.title}”.</p><p><a href="${origin}/chair/${grant.id}">Record the official decision</a></p>`,
